...
bool insert_function_redirect(void *function, void *redirect, uint32_t flags)
{
    ...
    // Fix permissions on the callsite.
    if (mprotect((void *)((uintptr_t) callsite & PAGE_MASK), PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC) != 0) {
        printf("mprotect() failed on target => %p (%m), try `sudo setenforce 0`\n", fixup);
        return false;
    }
