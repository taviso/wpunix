...
// This is a little tool to generate UTF-8 input sequences for wpterm.
//
// Usage: ./charset [charset num]
//
...
int main(int argc, char **argv)
{
    ...
    if (argc <= 1) {
        errx(EXIT_FAILURE, "You didn't specify a charset (1 - 11), Appendix P");
    }
    ...
    if (argc >= 3) {
        ...
        lim = charsets[set].size / sizeof(uint32_t) - 1;
    }

    if (set < 1 || set > 11) {
        errx(EXIT_FAILURE, "You specified an invalid charset");
    }

    if (lim > charsets[set].size / sizeof(uint32_t)) {
        errx(EXIT_FAILURE, "You specified an invalid limit");
    }

    for (int i = 0; i <= lim; i++) {
        uint32_t inchar = __builtin_bswap32(charsets[set].data[i]);
        ...
        if (inchar == 0) {
            //warnx("character %03d is not defined", i);
            continue;
        }
        ...
        if (iconv(cd, &inptr,
                      ...
                      &outptr,
                      &outsize) == -1) {
            err(EXIT_FAILURE, "cannot convert character %i", i);
        }

        // If it's just a space, leave it blank.
        if (strcmp(outbuf, " ") == 0)
            continue;

        // Just print it out to see what the table says.
        if (argc >= 4) {
            printf("N%03d '%s'\n", i, outbuf);
            continue;
        }
